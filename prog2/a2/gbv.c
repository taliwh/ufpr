...
int gbv_create(const char *filename) {
  ...
  bloco sb = {0};
  sb.qtd = 0;
  sb.offset = sizeof(sb) + sizeof(senha); 
...
int gbv_open(Library *lib, const char *chave, const char *filename) {
  if (!lib ||!filename || !chave) {
  ...
  senha senhacerta;
  if (fread(&senhacerta, sizeof(senha), 1, f) != 1) { 
    perror("erro: falha ao ler arquivo");
    return -1;
  }

  if (strcmp(chave, senhacerta.chave) != 0) {
...
int gbv_add(Library *lib, const char *archive, const char *docname) {
  ...
  if (gbv_data(lib, a, doc, docname, sb.offset)) {
    fclose(doc);
    fclose(f_lib);
    return -1;
  }
  ...
  fseek(f_lib, sizeof(senha), SEEK_SET);
